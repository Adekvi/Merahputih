<x-app-layout>
  ...
  <div class="bg-gray-100 dark:bg-gray-900 min-h-screen transition-colors duration-300">
    ...
    <div class="max-w-5xl mx-auto mt-10 p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg transition-colors duration-300">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div>
          ...
          <select id="selectKec" class="w-full border rounded p-2 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100">
        ...
        <div>
          ...
          <select id="selectDesa" class="w-full border rounded p-2 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100" disabled>
            <option value="">-- Pilih Desa --</option>
          </select>
        </div>

        <div class="flex items-end">
          <button id="btnLoadAll" class="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded disabled:opacity-50 transition" disabled>
            Tampilkan Semua
          </button>
        </div>
      </div>
    ...
    <div class="max-w-6xl mx-auto mt-6 p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg transition">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div>
          ...
          <select id="selectJenis" class="w-full border rounded p-2 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100">
        ...
        <div>
          ...
          <select id="selectKomoditas" class="w-full border rounded p-2 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100" disabled>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (selectKec) {
    selectKec.addEventListener('change', async function() {
      ...
      desas.forEach(d => {
        let val, label;
        if (typeof d === 'string') { val = label = d; }
        else if (d && d.desa) { val = label = d.desa; }
        else if (d && d.name) { val = label = d.name; }
        else { return; }
  ...
  if (selectDesa) {
    selectDesa.addEventListener('change', async function() {
      ...
      const url = `/api/jenis-potensi?kecamatan=${encodeURIComponent(kec)}&desa=${encodeURIComponent(desa)}`;
      const jenis = await safeFetchJson(url) || [];

      // expecting array of strings
      jenisContainer.innerHTML = '';
      if (!Array.isArray(jenis) || jenis.length === 0) {
        jenisContainer.innerHTML = '<div class="text-sm text-gray-500">Tidak ada jenis potensi terdaftar.</div>';
      ...
      jenis.forEach(j => {
        ...
        btn.type = 'button';
        btn.className = 'px-3 py-1 bg-gray-200 dark:bg-gray-600 rounded-lg text-gray-800 dark:text-gray-100 hover:bg-blue-500 hover:text-white transition mr-2 mb-2';
  ...
  async function loadProduksi(kec, desa, type) {
    ...
    const params = new URLSearchParams({ kecamatan: kec, desa: desa });
  ...
  function renderSummary(data) {
    ...
    data.forEach(group => {
      const typeLabel = group.type ? capitalize(String(group.type).replace(/_/g, ' ')) : 'Umum';
      const title = document.createElement('h3');
      title.className = 'text-lg font-semibold text-blue-700 dark:text-blue-400 mt-3 capitalize';
      ...
      if (Array.isArray(group.crops) && group.crops.length) {
        group.crops.forEach(c => {
          ...
          p.innerHTML = `<strong>${escapeHtml(c.nama ?? c.nama_tanaman ?? '')}</strong>` +
      ...
      if (Array.isArray(group.livestocks) && group.livestocks.length) {
        group.livestocks.forEach(l => {
          const p = document.createElement('p');
          p.innerHTML = `<strong>${escapeHtml(l.nama ?? l.jenis_ternak ?? '')}</strong>` +
                        ` — ${formatNumberLocale(l.jumlah)} ekor` +
                        (l.produksi_note || l.produksi ? ` ${escapeHtml('(' + (l.produksi_note ?? l.produksi) + ')')}` : '');
          container.appendChild(p);
        });
      }
  ...
  if (btnLoadAreas) {
    btnLoadAreas.addEventListener('click', async () => {
      ...
      if (resultAreas) resultAreas.innerHTML = '<p class="text-center text-gray-400">Memuat data...</p>';
      const url = `/api/desa-by-komoditas?jenis=${encodeURIComponent(jenis)}&komoditas=${encodeURIComponent(komoditas)}`;
      const data = await safeFetchJson(url) || [];
  ...
  function renderAreas(data, jenis, komoditas) {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      resultAreas.innerHTML = `<p class="text-center text-gray-400">Belum ada desa penghasil ${escapeHtml(komoditas)} (${escapeHtml(jenis)}).</p>`;
      return;
    }
    ...
    data.forEach(d => {
      ...
      title.innerHTML = `${escapeHtml(d.desa ?? d.name ?? '')}, <span class="text-sm text-gray-500">${escapeHtml(d.kecamatan ?? '')}</span>`;
      wrapper.appendChild(title);

      const detail = document.createElement('p');
      detail.className = 'text-sm ml-2';
      if (jenis === 'peternakan' || jenis === 'livestocks') {
        detail.textContent = `Jumlah: ${formatNumberLocale(d.jumlah)} ekor${d.produksi_note ? ' | ' + d.produksi_note : ''}`;
      } else {
        detail.textContent = `Luas: ${formatNumberLocale(d.luas)} ha | Produksi: ${formatNumberLocale(d.produksi)} ton`;
      }
